...
export class FillInTheBlankComponent {
    ...
    async mount() {
        ...
        AutocompleteService.setupForComponent(this, "answerWord", "autocompleteDropdown", (selectedItem) => {
            this.currentAnswerWordId = selectedItem.id;
            this.loadExistingQuestions();
            const input = document.getElementById("answerWord");
            const hiddenInput = document.getElementById("answerWordId");
    ...
    setupFieldDependencies() {
        const languageSelect = document.getElementById("answerLanguage");
        const answerInput = document.getElementById("answerWord");
        const hiddenAnswerId = document.getElementById("answerWordId");
        const clearBtn = document.getElementById("clearAnswerBtn");
        const distractorInput = document.getElementById("distractorInput");
        const createBtn = document.querySelector(".create-question-btn");
    ...
    populateLanguageDropdown() {
        const select = document.getElementById("answerLanguage");
        if (select) {
            select.innerHTML = `
        <option value="" disabled selected>Language</option>
        ${this.languages.map(lang => `<option value="${lang.language_id}">${lang.language_name}</option>`).join('')}
      `;
        }
    }
    async loadLanguages() {
        try {
            ...
            this.languages = (await response).map((lang) => ({
    ...
    async createFillBlank() {
        ...
        const sentenceInput = document.getElementById("questionSentence");
        ...
        let sentence = sentenceInput === null || sentenceInput === void 0 ? void 0 : sentenceInput.value.trim();
